...
export function HighlightedText({ text, terms, selected, onSelect }: Props) {
  ...
  return (
    <div className="whitespace-pre-wrap rounded-2xl border border-stone bg-white p-5 font-mono text-[15px] leading-8">
        ...
        return (
          <button
            ...
            className={`rounded px-0.5 underline decoration-2 underline-offset-4 transition ${KIND_STYLES[term.kind].mark} ${
              isSelected ? "ring-2 ring-ink" : "hover:brightness-95"
            }`}
          >
            {segment.text}
          </button>
